...
J+= lambda*(sum((reg_Theta1.^2)(:)) + sum((reg_Theta2.^2)(:)))/(2*m);



e3 = a3 - Y; %5000*10
%  (e2) = e3*Theta2.*(a(2)*(1-a(2))) 
e2 = (e3*Theta2).*(a2.*(1-a2));  % 5000*26
e2 = e2(:, 2:end);
...
p1 = (lambda/m)*([zeros(hidden_layer_size, 1), reg_Theta1]);
p2 = (lambda/m) *([zeros(num_labels, 1), reg_Theta2]);
